...
describe("Test chainfund contract", ()=>{
    ...
    beforeEach(async()=>{
        ...
        startingContractBalance = await audiophileContract.getBalance();
        amount = ethers.parseEther("0.001");
        await audiophileContract.fund(productId, {value: amount})
        endingContractBalance = await audiophileContract.getBalance();
    })
    ...
    describe("Test funding", ()=>{
        it("Successfully funded", async()=>{    
            ...
            assert.equal(Number(endingContractBalance), Number(startingContractBalance) + Number(amount))
        })
